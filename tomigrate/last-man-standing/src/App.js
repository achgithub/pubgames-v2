...
function App() {
  ...
  const [adminBackdoor, setAdminBackdoor] = useState(false);
  ...
  const handleAdminBackdoor = (password) => {
    axios.defaults.headers.common["X-Admin-Override"] = password;
    setAdminBackdoor(true);
    setUser({ id: 0, name: "Admin", is_admin: true });
    setView("admin");
  };
  ...
  const loadUserGameStatus = async (userId, gameId) => {
    try {
      const response = await axios.get(`${API_BASE}/games/status?user_id=${userId}&game_id=${gameId || ''}`);
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      alert(`Registration successful! Your login code is: ${response.data.code}\n\nPlease save this code to log in.`);
      setView('login');
    } catch (error) {
  ...
  const handleAdminRegister = async (e) => {
    ...
    const formData = new FormData(e.target);
    try {
      const response = await axios.post(`${API_BASE}/register/admin`, {
        email: formData.get('email'),
        name: formData.get('name'),
        password: formData.get('password')
      });
      alert(`Admin registration successful! Your login code is: ${response.data.code}\n\nPlease save this code to log in.`);
      setView('login');
    } catch (error) {
  ...
  const loadOpenRounds = async () => {
    if (!currentGame) return;
    const response = await axios.get(`${API_BASE}/rounds/open?user_id=${user.id}&game_id=${currentGame.id}`);
  ...
  const loadUsedTeams = async () => {
    ...
    try {
      const response = await axios.get(`${API_BASE}/predictions/used-teams?user_id=${user.id}&game_id=${currentGame.id}`);
  ...
  const loadPredictions = async (viewAll = false) => {
    ...
    try {
      ...
      const response = await axios.get(`${API_BASE}/predictions?user_id=${user.id}&game_id=${currentGame.id}${viewAllParam}`);
  ...
  // ============================================================================
  // LOGIN/REGISTER VIEW - Uses Shared CSS
  // ============================================================================
      ...
      <>
        ...
        <div className="container">
          <div className="auth-box">
            {config.logo_url && (
              <img src={config.logo_url} alt="Logo" style={{maxWidth: '150px', margin: '0 auto 20px', display: 'block'}} />
            )}
            ...
            {view === 'login' && (
              <>
                ...
                <p className="admin-link">
                  <a onClick={() => {
                    const pwd = prompt('Enter admin backdoor password:');
                    if (pwd) handleAdminBackdoor(pwd);
                  }}>🔐 Admin Backdoor</a>
                </p>
                        ...
                        return (
                          <div key={match.id} className="game-card">
                            ...
                            <div className="teams">
                              <button 
                                onClick={() => {
                                  // Re-check deadline at moment of click
          ...
          <div className="round-summary">
            ...
            {!summary ? (
              <div>
                ...
                <div className="rounds-list">
                  {rounds.filter(r => {
                    // Show rounds that are closed OR past deadline
